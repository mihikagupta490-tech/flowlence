...
const friendlyAuthError = (message: string) => {
  ...
  if (normalized.includes("invalid login") || normalized.includes("invalid credentials")) return "Email or password is incorrect.";
  if (normalized.includes("already registered") || normalized.includes("already been registered")) return "An account with this email already exists. Try signing in.";
  if (normalized.includes("email not confirmed")) return "Please confirm your email address before signing in.";
  if (normalized.includes("password")) return "Please choose a valid password and try again.";
...
function AuthPage({ mode, go }: { mode: "login" | "signup" | "recovery"; go: (path: string) => void }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (isRecovery) {
      ...
      setLoading(false);
      if (recoveryError) {
        setError(friendlyAuthError(recoveryError.message));
        return;
      }
...
export default function Home() {
  ...
  const refresh = async (userId: string) => {
    ...
    try {
      ...
      setDataError("We could not load your financial data. Please try again.");
    } finally {
  ...
  const saveTransaction = async (data: Omit<Transaction, "id">, id?: string | number) => {
    ...
    try {
      ...
      setDataError("We could not save this transaction. Please check your entries and try again.");
